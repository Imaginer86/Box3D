...
b3ContactFilter b3_defaultFilter;
b3ContactListener b3_defaultListener;
...
void b3ContactManager::Collide()
{
	...
	while (c)
	{
		...
		if (c->m_flags & b3Contact::e_filterFlag)
		{
			...
			c->m_flags &= ~b3Contact::e_filterFlag;
		}

		bool activeA = bodyA->IsAwake() && bodyA->m_type != b3_staticBody;
		bool activeB = bodyB->IsAwake() && bodyB->m_type != b3_staticBody;
...
void b3ContactManager::AddPair(void* proxyUserDataA, void* proxyUserDataB)
{
	b3FixtureProxy* proxyA = (b3FixtureProxy*)proxyUserDataA;
	b3FixtureProxy* proxyB = (b3FixtureProxy*)proxyUserDataB;

	b3Fixture* fixtureA = proxyA->fixture;
	b3Fixture* fixtureB = proxyB->fixture;
	...
	while (edge)
	{
		if (edge->other == bodyA)
		{
			b3Fixture* fA = edge->contact->GetFixtureA();
			b3Fixture* fB = edge->contact->GetFixtureB();
	...
	// Call the factory.
	b3Contact* c = b3Contact::Create(fixtureA, indexA, fixtureB, indexB, m_allocator);
	if (c == nullptr)
	{
		return;
	}
