...
static void sKeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (action == GLFW_PRESS)
	{
		...
		case GLFW_KEY_Z:
			// Zoom out
			g_camera.m_zoom = b3Min(1.1f * g_camera.m_zoom, 20.0f);
			break;

		case GLFW_KEY_X:
			// Zoom in
			g_camera.m_zoom = b3Max(0.9f * g_camera.m_zoom, 0.02f);
			break;
...
static void sMouseButton(GLFWwindow* window, int32 button, int32 action, int32 mods)
{
	...
	b3Vec2 ps((float32)xd, (float32)yd);
	...
	if (button == GLFW_MOUSE_BUTTON_1)
	{
		//<##>
		//ps.Set(0, 0);
		b3Vec2 pw = g_camera.ConvertScreenToWorld(ps);
		if (action == GLFW_PRESS)
		{
...
int main(int, char**)
{
	...
	char title[64] = "Box3D Testbend";
	//sprintf(title, "Box3D Testbed Version %d.%d.%d", b3_version.major, b3_version.minor, b3_version.revision);
	...
	testIndex = b3Clamp(testIndex, 0, testCount - 1);
	testSelection = testIndex;
